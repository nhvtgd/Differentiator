...
public class Parser {
	...
	/**
	 * Follow the actual grammar rule , E() is non-terminal E = "(" T [+|-] T +
	 * ")" | "(" T ")" T = Num|Var|E modifies expression
	 * 
	 * @return Expression
	 * */
		...
		if (next.getType().equals(Token.Type.LEFTPAREN)) {
			...
			if (next2.getType().equals(Token.Type.RIGHTPAREN)) {
				...
				if (currentToken.getType().equals(Token.Type.RIGHTPAREN)) {
					...
					default:
						JOptionPane.showMessageDialog(null, "Illegal TOKEN "
								+ next2.getType());
					...
					JOptionPane.showMessageDialog(null, "Expected Rightparen");
					throw new RuntimeException("Expected Rightparen");
				}

			} else {
				JOptionPane.showMessageDialog(null,
						"Missing right paren or operator");
				throw new RuntimeException("Missing right paren or operator");
			}
		} else {
			JOptionPane
					.showMessageDialog(null, "Expected Opening parenthesis ");
